...
- **Duplikat `(pengguna, desa, peran)` aktif/menunggu → `409`.** Untuk pengajuan yang **`ditolak`/`revisi`**, pengguna boleh submit ulang kombinasi yang sama; backend me-*flip* status baris lama ke `menunggu` (bukan membuat baris baru). UI tampilkan sebagai revisi/submit ulang.
...
4. **Hasil:**
   ...
   - `409` → state "kamu sudah punya/sedang mengajukan peran ini di desa ini" + arahkan ke status.

## State "menunggu persetujuan" & daftar keanggotaan

Halaman/section **Status Keanggotaan** (reuse di Akun, `GET /saya/keanggotaan`): daftar peran lintas-desa dengan badge status — **aktif · menunggu · revisi · ditolak · nonaktif** — dan desa terkait. `menunggu` → tampil "sedang ditinjau"; `revisi`/`ditolak` → tampil alasan bila ada + arahan untuk revisi dan submit ulang lewat form Gabung Komunitas. Ini pintu masuk pengguna memantau pengajuannya.
...
Field motivasi/dokumen pada pengajuan (butuh perluasan endpoint) · catatan/alasan revisi yang terstruktur (bila belum tersedia di API) · notifikasi saat pengajuan disetujui/ditolak (butuh modul notifikasi — celah backend). Semua ditampilkan apa adanya, tanpa mengarang perilaku.
