"""Gerbang destinasi: unik, soft delete, visibilitas publik vs kelola, validasi."""
import pytest
...
def test_slug_unik_per_desa(store, desa, svc_destinasi):
    k = _pengelola(store, desa)
    svc_destinasi.buat(k, desa.id, _data("gigi-hiu"))
    with pytest.raises(Konflik):
        svc_destinasi.buat(k, desa.id, _data("gigi-hiu"))


def test_soft_delete_sembunyi_dari_publik(store, desa, svc_destinasi):
    k = _pengelola(store, desa)
    d = svc_destinasi.buat(k, desa.id, _data("laguna"))
    svc_destinasi.hapus(k, desa.id, d.id)
    with pytest.raises(TidakDitemukan):
        svc_destinasi.detail(Konteks(), desa.id, d.id)
    assert svc_destinasi.cari(Konteks(), desa.id)["item"] == []


def test_draft_hanya_terlihat_pengelola(store, desa, svc_destinasi):
    k = _pengelola(store, desa)
    d = svc_destinasi.buat(k, desa.id, _data("draf-spot", status=StatusKonten.draft))
    # publik tak melihat draft
    assert svc_destinasi.cari(Konteks(), desa.id)["item"] == []
    with pytest.raises(TidakDitemukan):
        svc_destinasi.detail(Konteks(), desa.id, d.id)
    # pengelola melihatnya
    assert len(svc_destinasi.cari(k, desa.id)["item"]) == 1
    assert svc_destinasi.detail(k, desa.id, d.id).id == d.id
