"""Destinasi Kiluan — manajemen destinasi + pencarian publik.

Aturan kunci: filter tenant wajib; lintas-tenant → 404; publik hanya melihat
`publikasi` & belum soft-deleted; publikasi butuh peran pengelola.
"""
...
class DestinasiLayanan:
    ...
    # --- tulis ---
    def buat(self, konteks: ctx.Konteks, desa_id: UUID, data: DestinasiBuat) -> Destinasi:
        ctx.wajib(konteks, rbac.KELOLA_DESTINASI, desa_id)
        if self.store.kategori.ambil(data.kategori_id) is None:
        ...
        d = Destinasi(
            ...
            lokasi=(data.lokasi.lat, data.lokasi.lng),
        ...
        return self.store.destinasi.tambah(d)  # (desa_id, slug) unik → Konflik

    def _ambil_milik(self, desa_id: UUID, id: UUID, *, publik: bool) -> Destinasi:
        d = self.store.destinasi.ambil(id)
    ...
    def ubah(self, konteks: ctx.Konteks, desa_id: UUID, id: UUID, data: DestinasiUbah) -> Destinasi:
        ctx.wajib(konteks, rbac.KELOLA_DESTINASI, desa_id)
        d = self._ambil_milik(desa_id, id, publik=False)
        ...
        d.diperbarui_pada = _now()
        return d

    def ubah_status(self, konteks: ctx.Konteks, desa_id: UUID, id: UUID, status: StatusKonten) -> Destinasi:
        ctx.wajib(konteks, rbac.KELOLA_DESTINASI, desa_id)
        if status == StatusKonten.publikasi:
            ctx.wajib(konteks, rbac.PUBLIKASI_DESTINASI, desa_id)
        d = self._ambil_milik(desa_id, id, publik=False)
    ...
    def hapus(self, konteks: ctx.Konteks, desa_id: UUID, id: UUID) -> None:
        ctx.wajib(konteks, rbac.KELOLA_DESTINASI, desa_id)
        d = self._ambil_milik(desa_id, id, publik=False)
        d.dihapus_pada = _now()  # soft delete

    # --- baca ---
    def detail(self, konteks: ctx.Konteks, desa_id: UUID, id_atau_slug) -> Destinasi:
        publik = not ctx.boleh(konteks, rbac.KELOLA_DESTINASI, desa_id)
        d = None
        if isinstance(id_atau_slug, UUID):
            d = self.store.destinasi.ambil(id_atau_slug)
        else:
            d = self.store.destinasi.ambil_slug(desa_id, id_atau_slug)
        if d is None or d.desa_id != desa_id or d.dihapus_pada is not None:
            raise TidakDitemukan("destinasi tidak ditemukan")
        if publik and d.status != StatusKonten.publikasi:
            raise TidakDitemukan("destinasi tidak ditemukan")
    ...
    def cari(
        ...
        *,
        ...
        publik = not ctx.boleh(konteks, rbac.KELOLA_DESTINASI, desa_id)
        rows = [d for d in self.store.destinasi.daftar(desa_id) if d.dihapus_pada is None]
        if publik:
        ...
        if q:
            ...
            rows = [
                d for d in rows
                if ql in d.nama.lower() or (d.deskripsi and ql in d.deskripsi.lower())
            ]
        if dekat is not None:
            ...
            for d in rows:
                ...
                if radius_m is None or jm <= radius_m:
                    hasil.append((d, jm))
            hasil.sort(key=lambda x: x[1])  # terurut jarak asc
