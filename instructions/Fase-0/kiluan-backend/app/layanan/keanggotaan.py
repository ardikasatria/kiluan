"""Balai Warga — keanggotaan (apply peran per-desa + persetujuan pengelola)."""
...
# Peran yang boleh auto-aktif tanpa persetujuan saat apply.
...
class KeanggotaanLayanan:
    ...
    def ajukan(self, konteks: ctx.Konteks, desa_id: UUID, peran: KodePeran) -> Keanggotaan:
        ...
        status = (
            StatusKeanggotaan.aktif if peran in _AUTO_AKTIF else StatusKeanggotaan.menunggu
        )
        k = Keanggotaan(
        ...
        return self.store.keanggotaan.tambah(k)  # duplikat → Konflik

    def daftar(self, konteks: ctx.Konteks, desa_id: UUID, peran=None, status=None):
        ctx.wajib(konteks, rbac.KELOLA_KEANGGOTAAN, desa_id)
        return self.store.keanggotaan.daftar_desa(desa_id, peran=peran, status=status)

    def putuskan(
        self, konteks: ctx.Konteks, desa_id: UUID, keanggotaan_id: UUID, status_baru: StatusKeanggotaan
    ) -> Keanggotaan:
        ctx.wajib(konteks, rbac.KELOLA_KEANGGOTAAN, desa_id)
        k = self.store.keanggotaan.ambil(keanggotaan_id)
