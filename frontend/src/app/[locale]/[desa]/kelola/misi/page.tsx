import MisiKelolaClient from '@/components/kiluan/penjelajah/MisiKelolaClient'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations('kelola.misi')
...
export default async function MisiKelolaPage({ params }: Props) {
  const { desa } = await params
  const t = await getTranslations('kelola.misi')
  const profil = await getProfilDesa(desa)
  if (!profil) notFound()
  ...
  return (
    <div className="space-y-6">
      ...
      <MisiKelolaClient desaSlug={desa} />
    </div>
  )
}
