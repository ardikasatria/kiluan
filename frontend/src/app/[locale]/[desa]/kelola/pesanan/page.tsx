import PesananKelolaClient from '@/components/kiluan/dermaga/PesananKelolaClient'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations('kelola.pesanan')
...
export default async function PesananKelolaPage({ params }: Props) {
  const { desa } = await params
  const t = await getTranslations('kelola.pesanan')
  const profil = await getProfilDesa(desa)
  if (!profil) notFound()
  ...
  return (
    <div className="space-y-6">
      ...
      <PesananKelolaClient desaSlug={desa} desaId={profil.id} />
    </div>
  )
}
