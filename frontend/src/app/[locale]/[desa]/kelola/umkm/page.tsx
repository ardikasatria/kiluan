import VerifikasiUmkmClient from '@/components/kiluan/pasar/VerifikasiUmkmClient'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations('kelola.umkm')
...
export default async function VerifikasiUmkmPage({ params }: Props) {
  const { desa } = await params
  const t = await getTranslations('kelola.umkm')
  const profil = await getProfilDesa(desa)
  if (!profil) notFound()
  ...
  return (
    <div className="space-y-6">
      ...
      <VerifikasiUmkmClient desaSlug={desa} />
    </div>
  )
}
