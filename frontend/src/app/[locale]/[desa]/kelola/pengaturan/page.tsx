import PengaturanKelolaClient from '@/components/kiluan/dermaga/PengaturanKelolaClient'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations('kelola.pengaturan')
...
export default async function PengaturanKelolaPage({ params }: Props) {
  const { desa } = await params
  const t = await getTranslations('kelola.pengaturan')
  const profil = await getProfilDesa(desa)
  if (!profil) notFound()
  ...
  return (
    <div className="space-y-6">
      ...
      <PengaturanKelolaClient desaSlug={desa} />
    </div>
  )
}
