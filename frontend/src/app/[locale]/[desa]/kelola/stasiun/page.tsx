import StasiunKelolaClient from '@/components/kiluan/penjelajah/StasiunKelolaClient'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations('kelola.stasiun')
...
export default async function StasiunKelolaPage({ params }: Props) {
  const { desa } = await params
  const t = await getTranslations('kelola.stasiun')
  const profil = await getProfilDesa(desa)
  if (!profil) notFound()
  ...
  return (
    <div className="space-y-6">
      ...
      <StasiunKelolaClient desaSlug={desa} />
    </div>
  )
}
