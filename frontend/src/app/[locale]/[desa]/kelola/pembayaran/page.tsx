...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations('kelola.pembayaran')
...
export default async function PembayaranKelolaPage({ params }: Props) {
  const { desa } = await params
  const t = await getTranslations('kelola.pembayaran')
  const profil = await getProfilDesa(desa)
  if (!profil) notFound()
  ...
  return (
    <div className="space-y-6">
      ...
      <BendaharaClient desaSlug={desa} mode="pembayaran" />
    </div>
  )
}
