import SlotKelolaClient from '@/components/kiluan/dermaga/SlotKelolaClient'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations('kelola.slot')
...
export default async function SlotKelolaPage({ params }: Props) {
  const { desa } = await params
  const t = await getTranslations('kelola.slot')
  const profil = await getProfilDesa(desa)
  if (!profil) notFound()
  ...
  return (
    <div className="space-y-6">
      ...
      <SlotKelolaClient desaSlug={desa} />
    </div>
  )
}
