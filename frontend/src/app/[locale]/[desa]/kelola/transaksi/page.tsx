import TransaksiKelolaClient from '@/components/kiluan/dermaga/TransaksiKelolaClient'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations('kelola.transaksi')
...
export default async function TransaksiKelolaPage({ params }: Props) {
  const { desa } = await params
  const t = await getTranslations('kelola.transaksi')
  const profil = await getProfilDesa(desa)
  if (!profil) notFound()
  ...
  return (
    <div className="space-y-6">
      ...
      <TransaksiKelolaClient desaSlug={desa} />
    </div>
  )
}
