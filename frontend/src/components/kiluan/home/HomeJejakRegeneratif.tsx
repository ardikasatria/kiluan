import { ChartBarIcon, ShieldCheckIcon } from '@heroicons/react/24/outline'

export default function HomeJejakRegeneratif() {
  return (
    <section id="jejak-regeneratif" className="scroll-mt-24 border-y border-neutral-200/70 bg-white/35 py-16 backdrop-blur-sm dark:border-neutral-800/70 dark:bg-neutral-900/25 sm:py-20">
      <div className="container">
        <div className="mx-auto max-w-3xl text-center">
          <p className="inline-flex items-center gap-2 text-sm font-medium text-primary-600 dark:text-primary-400">
            <ChartBarIcon className="size-4" aria-hidden />
            Jejak Regeneratif
          </p>
          <h2 className="mt-2 text-2xl font-bold text-primary-800 sm:text-3xl dark:text-primary-100">
            Transparansi dampak konservasi
          </h2>
          <p className="mt-4 leading-relaxed text-neutral-600 dark:text-neutral-400">
            sigerciv berkomitmen menampilkan jejak konservasi yang terverifikasi — dari dana konservasi hingga
            indikator daya dukung. Angka dampak hanya akan ditampilkan setelah data monitoring terverifikasi
            tersedia, bukan sebagai klaim aspirasional.
          </p>
        </div>

        <div className="mx-auto mt-10 grid max-w-4xl gap-5 sm:grid-cols-2">
          <div className="rounded-2xl border border-neutral-200 bg-white p-6 text-center dark:border-neutral-700 dark:bg-neutral-800/60">
            <div className="mx-auto flex size-12 items-center justify-center rounded-xl bg-kiluan-mint/20 text-primary-700 dark:bg-primary-900/50 dark:text-kiluan-mint">
              <ShieldCheckIcon className="size-6" aria-hidden />
            </div>
            <h3 className="mt-4 font-semibold text-primary-800 dark:text-primary-100">Komitmen terbuka</h3>
            <p className="mt-2 text-sm text-neutral-600 dark:text-neutral-400">
              Dana konservasi dan neraca regeneratif akan dipublikasikan saat pipeline data F3 aktif.
            </p>
          </div>
          <div className="rounded-2xl border border-dashed border-primary-300/60 bg-primary-50/40 p-6 text-center dark:border-primary-600/40 dark:bg-primary-900/20">
            <p className="text-sm font-medium text-primary-700 dark:text-primary-300">Target transparansi</p>
            <p className="mt-2 text-sm text-neutral-600 dark:text-neutral-400">
              Transparansi menyusul saat data konservasi terverifikasi — tanpa angka hard-code di halaman ini.
            </p>
          </div>
        </div>
      </div>
    </section>
  )
}
