...
class DestinasiLayanan:
    ...
    async def cari(self, konteks: ctx.Konteks, desa_id: UUID, *, kategori_id: Optional[int] = None,
        ...
        if dekat is not None:
            lat, lng = dekat
            if hasattr(self.store.destinasi, "cari_dekat"):
                return await self.store.destinasi.cari_dekat(
                    desa_id, lat=lat, lng=lng, radius_m=radius_m, publik=publik,
                    kategori_id=kategori_id, tag=tag, q=q, batas=batas,
                )
            hasil = []
            for d in rows:
