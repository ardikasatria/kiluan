...
@pytest.mark.asyncio
async def test_transisi_ilegal_kontribusi(dapur, desa, aktor_warga, aktor_kontributor):
    k = await dapur.kirim(aktor_warga, desa.id, {
        "tipe": "tips", "target_tipe": "desa", "muatan": {"isi": "tips bagus"},
    })
    await dapur.transisi(aktor_kontributor, desa.id, k.id, "setuju")
    with pytest.raises(TransisiIlegal):
        await dapur.transisi(aktor_kontributor, desa.id, k.id, "minta_revisi")


@pytest.mark.asyncio
async def test_setuju_dua_kali_poin_idempoten(dapur, store, desa, aktor_warga, aktor_kontributor):
...
@pytest.mark.asyncio
async def test_kontribusi_setuju_menulis_log(dapur, store, desa, aktor_warga, aktor_kontributor):
    k = await dapur.kirim(aktor_warga, desa.id, {
...
@pytest.mark.asyncio
async def test_transisi_kontribusi_revisi_ajukan(dapur, desa, aktor_warga, aktor_kontributor):
    k = await dapur.kirim(aktor_warga, desa.id, {
        "tipe": "tips", "target_tipe": "desa", "muatan": {"isi": "v1"},
    })
    await dapur.transisi(aktor_kontributor, desa.id, k.id, "minta_revisi", "perbaiki")
