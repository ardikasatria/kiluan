...
@pytest_asyncio.fixture
async def aktor_kontributor(store, desa):
...
@pytest.mark.asyncio
async def test_publik_hanya_publikasi_terbit(warta, desa, aktor_kontributor, aktor_wisatawan):
    masa_depan = datetime.now(timezone.utc) + timedelta(days=7)
    b = await warta.buat(aktor_kontributor, desa.id, {
        "slug": "jadwal", "judul": "Artikel Terjadwal", "konten": "isi",
    })
    await warta.ubah_status(aktor_kontributor, desa.id, b.id, "publikasi", terbit_pada=masa_depan)

    publik = await warta.daftar(aktor_wisatawan, desa.id)
    assert len(publik["item"]) == 0

    kelola = await warta.daftar(aktor_kontributor, desa.id, status="publikasi")
    assert len(kelola["item"]) == 1


@pytest.mark.asyncio
async def test_slug_duplikat_409(warta, desa, aktor_kontributor):
    await warta.buat(aktor_kontributor, desa.id, {"slug": "sama", "judul": "A", "konten": "x"})
    with pytest.raises(Konflik):
        await warta.buat(aktor_kontributor, desa.id, {"slug": "sama", "judul": "B", "konten": "y"})


@pytest.mark.asyncio
async def test_soft_delete_hilang_publik(warta, desa, aktor_kontributor, aktor_wisatawan):
    b = await warta.buat(aktor_kontributor, desa.id, {"slug": "hapus", "judul": "H", "konten": "x"})
    await warta.ubah_status(aktor_kontributor, desa.id, b.id, "publikasi")
    await warta.hapus(aktor_kontributor, desa.id, b.id)
    publik = await warta.daftar(aktor_wisatawan, desa.id)
    assert len(publik["item"]) == 0
...
@pytest.mark.asyncio
async def test_keyset_stabil(warta, desa, aktor_kontributor, aktor_wisatawan):
    for i in range(3):
        b = await warta.buat(aktor_kontributor, desa.id, {
...
@pytest.mark.asyncio
async def test_detail_slug_publik(warta, desa, aktor_kontributor, aktor_wisatawan):
    await warta.buat(aktor_kontributor, desa.id, {"slug": "by-slug", "judul": "S", "konten": "k"})
